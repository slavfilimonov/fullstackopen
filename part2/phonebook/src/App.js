...
const App = () => {
  ...
  const sameValues = () => {
    const sameName = !!persons.find(person => isEqualJSON(person.name, nameInput))
    const sameNumber = !!persons.find(person => isEqualJSON(person.number, numberInput))
  ...
  const addPerson = (event) => {
    ...
    switch (sameValues()) {
      case 'name':
        const id = persons.find(person => person.name === nameInput).id
  ...
  return (
    <>
      ...
      />
      <h3>Numbers</h3>
      <Persons persons={persons} filter={filterInput} onClick={deletePerson} />
    </>
  )
}
